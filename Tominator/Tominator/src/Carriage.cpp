...
void Carriage::HandleDCMotor(int speed, int goToHallSensor)
{
	...
	if (!digitalRead(goToHallSensor))
	{
		...
		switch (goToHallSensor)
		{
			...
			case HALL_CARRIAGE_MIDDLE:
				...
				else if (digitalRead(this->hallSensorTopPin))
				{
					motorTurnedOn = true;
			...
			case HALL_CARRIAGE_TOP:
				if (digitalRead(this->hallSensorBottomPin) || digitalRead(this->hallSensorMiddlePin))
				{
					motorTurnedOn = true;
					this->GetDCMotor().Start(DirectionType::Forward);
				}
	...
	while (!digitalRead(goToHallSensor) && motorTurnedOn)
	{
		this->GetDCMotor().Run();
	}
